...
red: context [
	...
	;hao 这些是语言控制结构关键字，内置的语言关键词，应该也是用 boot.red 内的函数来实现的
	intrinsics:   [
	...
	;---name--type--arity----------spec----------------------------refs--
		...
		;hao make 是预定义的，跟 TYPE_VALUE 类似，需要预先加载
	]
	...
	fetch-functions: func [pos [block!] /local name type spec refs arity][
		...
		switch type: pos/3 [
			native! [if find intrinsics name [type: 'intrinsic!]] ;hao 内置函数，语言控制结构
	...
	comp-source: func [code [block!] /local user main][
		...
		;?? user				;-- output 被追加了，user 也跟着有了编译的结果
		;hao-globals 3 1
...
;		if find [1 2] verbose [probe user]
    ...
    hao-globals: func [
        ...
        print ["-----------" index "-----------"]
        if pc [
            prin "pc/1 --> " probe pc/1
            prin "pc/2 --> " probe pc/2
            prin "pc/3 --> " probe pc/3
            prin "pc/4 --> " probe pc/4
            prin "pc/5 --> " probe pc/5
        ]
        ;?? symbols
        ;?? sym-table
        ;?? globals
		...
		debug_file: to-rebol-file rejoin ["debug_output_" index ".log"]
		write debug_file mold output
