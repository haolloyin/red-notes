...
block: context [
	...
	rs-length?: func [
		...
		s: GET_BUFFER(blk) ;-- #define GET_BUFFER(series)  (as series! series/node/value)

		print-line ["005 block/rs-length? blk: " as-integer blk ", s: " as-integer s]
		print-line ["    s/tail: " as-integer s/tail ", s/offset: " as-integer s/offset]
		print-line ["    s/tail - s/offset: " (as-integer (s/tail - s/offset)) ", blk/head: " blk/head]

		;-- 右移 4 位相当于除以 16，因为 cell! 包含 4 个指针， 4 * 4 bytes = 16 bytes，见 alloc-cells
        ;-- 即计算是属于 red-block! 所指向的 series-buffer! 的第几个 cell!
		a: (as-integer (s/tail - s/offset)) >> 4
		b: a - blk/head
		print-line ["    a: " a ", b: " b]
		b
	...
	rs-head: func [
		...
		s: GET_BUFFER(blk)
		s/offset + blk/head ;-- series-buffer!/offset 指向第一个 cell
	]
	...
	append*: func [
        ...
        ;-- arg 是 red-block!，arg + 1 是下一个 cell
        ;-- 把 cell 复制到 block 的 buffer 指向的 cell 中
        ;-- 每次 append 之后，block 的 buffer/tail 将会 +1，
        ;-- 所以 pick 函数读取时地址不能大于 tail
		copy-cell
			as cell! arg + 1 ;-- 将要被 append 的 cell
			ALLOC_TAIL(arg)  ;-- 参数栈第一个参数是 block! 的 buffer
	...
	make-at: func [
		...
		blk/head: 	0
		blk/node: 	alloc-cells size	;-- blk 是 _root，即给 _root 指向 memory/n-active/top/value
		blk
	]
	...
	pick: func [
		...
		either any [
			zero? index
			cell >= s/tail ;-- 见 append*，每次新增后都会 tail +1，>= 则非法
			cell < s/offset
		][
			none-value
		][
	...
	poke: func [
		...
		either any [
			...
			copy-cell data cell
			stack/set-last data ;-- 把 data 写回 stack/arguments
		]
		as red-value! data
	]
